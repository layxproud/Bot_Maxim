        ...
        if re.match("макс шанс", received_message):
            if self.isAdmin(chat_id, from_id):
                if len(word_list) == 3 and int(word_list[-1]) in range (0, 101):
                    ...
                    self.sender(chat_id, f"Установлен шанс ответа {self.change_chance(chance)}")
                else:
                    self.sender(chat_id, "Ожидаемый ввод: max chance число_от_0_до_100")
            else:
              self.sender(chat_id, "Команда доступна только администраторам")
              
        elif re.match("доброе утро", received_message):  
            self.sender(chat_id, "Доброе утро, котенок &#128573;")
            
        elif re.match("спокойной ночи", received_message):  
            self.sender(chat_id, "Спокойной ночи, сладкий &#127800;")
            
        elif re.match("макс инфа", received_message):
            if len(word_list) > 2:
                self.sender(chat_id, f"Ммм, шанс этого {random.randrange(1, 100, 1)}")
            else:
                self.sender(chat_id, "Так что тебя конкретно интересует, сладкий?")
